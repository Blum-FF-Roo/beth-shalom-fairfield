...
// History content sections data
const historyContentSections = [
  {
    key: 'historyIntro',
...
export async function initHistoryContent(userId: string = 'system'): Promise<void> {
  console.log('Initializing history content sections...');
  ...
  try {
    for (const sectionData of historyContentSections) {
      ...
      console.log(`Created content section: ${sectionData.title}`);
    }
    
    console.log('✅ All history content sections created successfully!');
  } catch (error) {
    console.error('❌ Error creating history content sections:', error);
    throw error;
  }
}
...
if (require.main === module) {
  initHistoryContent().catch(console.error);
}